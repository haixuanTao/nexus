...
pub fn gpu_mb_integrate(
    ...
    #[spirv(storage_buffer, descriptor_set = 0, binding = 4)] dof_velocities: &mut [f32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 5)] gen_accelerations: &[f32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 6)] num_multibodies: &[u32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 7)] dt_buf: &[f32],
    #[spirv(uniform, descriptor_set = 0, binding = 8)] multibodies_batch_capacity: &u32,
    #[spirv(uniform, descriptor_set = 0, binding = 9)] links_batch_capacity: &u32,
    ...
    // v += a * dt for every DOF in this multibody.
    for d in 0..mb.ndofs {
        let di = d as usize;
    ...
    // Per-link coord / joint_rot update.
    for k in 0..num_links {
        let k_usize = k as usize;
        let stat = stat_slice.read(k_usize);
        let mut ws = ws_slice.read(k_usize);
