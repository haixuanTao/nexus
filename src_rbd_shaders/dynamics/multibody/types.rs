...
pub struct MultibodyInfo {
    ...
    /// builder uses this instead of a hardcoded default, so a collider's
    /// configured friction (and per-env friction randomization) actually
    /// reaches the GPU solver. Falls back to `FRICTION_DEFAULT` (0.5).
    pub friction: f32,
}
