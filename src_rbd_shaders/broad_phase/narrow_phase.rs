...
/// OPTIONAL contact reduction (training-grade). A convex collider resting on
/// a trimesh emits one manifold PER OVERLAPPING TRIANGLE — all sharing one
/// `colliders` key (the trimesh is always collider A, so their points share
/// one local frame). Downstream, the multibody contact-constraint kernels pay
/// 6 solver slots per point, so a foot on rough terrain costs 4–8× its
/// flat-ground price for near-duplicate information. This pass compacts the
/// batch's contacts in place: the FIRST record of each collider pair is kept
/// verbatim (single-manifold pairs — e.g. all flat-ground contacts — are
/// bit-identical to the unreduced path), and every further record with the
/// same key is MERGED into it — pool both manifolds' points (≤8), keep the
/// deeper manifold's normal, and run the standard deepest+spread selection
/// down to `MAX_MANIFOLD_POINTS`. Approximations (documented, opt-in for RL
/// training): one normal per merged manifold (a foot straddling a box edge
/// loses the second normal) and greedy pairwise merging in emission order.
/// Dispatched only when the host enables contact reduction; grid
/// `[1, num_batches, 1]`, serial per batch (n ≤ contacts capacity, ~128).
#[cfg(feature = "dim3")]
#[spirv_bindgen]
#[spirv(compute(threads(1)))]
...
pub fn gpu_narrow_phase_shape_shape(
    ...
    for i in StepRng::new(invocation_id.x..len, num_threads) {
        ...
        let pose1 = poses[pair.read(0) as usize];
        let pose2 = poses[pair.read(1) as usize];
        let shape1 = &shapes[pair.read(0) as usize];
        let shape2 = &shapes[pair.read(1) as usize];
