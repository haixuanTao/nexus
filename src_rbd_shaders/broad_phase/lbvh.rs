...
/// in indices [0..n-1[, leaf nodes in [n-1..2n-1[.
///
/// This means that each LBVH has 2n - 1 nodes.
///
/// To store multiple trees in the same buffer (for batch dimensions), we
/// simplify by using 2n node slots per tree (the last one is unused). This
/// simplified calculations of the tree’s starting point from the batch index
/// since that makes it so it doesn’t depend on the actual number of trees
/// stored before (only on the total number of nodes).
///
/// For internal nodes:
/// - left/right point to child nodes (can be internal or leaf)
