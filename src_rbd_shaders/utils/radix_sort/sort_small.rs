...
//!
//! The radix path costs `ceil(sorting_bits/4)` passes × (count + reduce + scan
//! + scan_add + scatter) launches, plus extra stable passes for batch-id bits
//! in the flattened multi-batch case — ~60 launches per broad-phase rebuild
//! for a robot-RL scene whose batches hold only a dozen colliders each.
//!
//! This kernel sorts each batch independently: one workgroup per batch loads
//! the batch's keys into shared memory, bitonic-sorts them, and writes the
//! result. Batch grouping is inherent (no batch-id passes), and no ping-pong /
//! histogram workspace is touched.
//!
//! **Stability**: bitonic networks are not stable, so the comparator orders by
//! `(key, original_index)` lexicographically. Equal keys therefore keep their
//! input order — the output is IDENTICAL to the stable radix path's, bit for
//! bit, including the position of sentinel-keyed inactive elements.
//!
//! Capacity: `SMALL_SORT_MAX` (128) elements per batch, padded in shared
//! memory to the next power of two with `u32::MAX` sentinel keys.
...
pub fn gpu_sort_small(
    ...
    let mut i = tid;
    while i < p {
        let k = if i < n { src.read(base + i as usize) } else { u32::MAX };
