...
pub fn render_ui(
    ...
    window.draw_ui(|ctx| {
        kiss3d::egui::Window::new("Settings").show(ctx, |ui| {
            ...
            // Print the warning at the end so it doesn’t change the layout when it appears/disappears.
            if matches!(backend_type, BackendType::Gpu { .. }) && run_stats.total_simulation_time_with_readback.as_secs_f32() > 0.1 {
                ...
                ui.colored_label(
                    ...
                    "⚠ running slow? If you have both an integrated and discrete GPU, ensure the discrete GPU is in use.".to_string(),
                    #[cfg(target_arch = "wasm32")]
                    "⚠ running slow? If you have both an integrated and discrete GPU, ensure your browser runs exclusively on the discrete GPU.".to_string(),
                );
            }
        });
    });
