...
// Only no_std when targeting GPU (spirv). On CPU, we need std for generated ShaderArgs.
...
//
// These functions wrap SPIR-V atomic intrinsics for use in compute shaders.
// They operate on mutable references within storage buffers.
//

/// Atomically adds `value` to the u32 at `ptr` and returns the old value.
///
/// This is the GPU equivalent of `fetch_add` for storage buffer counters.
#[inline]
pub fn atomic_add_u32(ptr: &mut u32, value: u32) -> u32 {
    // SAFETY: This uses SPIR-V atomic instructions which are safe when
    // operating on storage buffer memory with proper synchronization.
    unsafe {
...
/// Atomically computes the maximum of the u32 at `ptr` and `value`, storing the result.
...
/// Atomically computes the minimum of the u32 at `ptr` and `value`, storing the result.
/// Returns the old value.
#[inline]
...
/// Atomically adds `value` to the u32 at `ptr` and returns the old value.
/// This version is for workgroup-local (shared) memory.
#[inline]
pub fn atomic_add_u32_workgroup(ptr: &mut u32, value: u32) -> u32 {
    // SAFETY: This uses SPIR-V atomic instructions which are safe when
    // operating on workgroup-local memory with proper synchronization.
    unsafe {
        spirv_std::arch::atomic_i_add::<
            u32,
