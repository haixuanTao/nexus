...
/// `NEXUS_ENV_PER_LANE=1`: run the tree-sparse LᵀDL factor + gravity solve
/// one-multibody-per-LANE (`gpu_mb_gravity_rhs` + `gpu_mb_ltdl_lanes`)
...
impl GpuMultibodySolver {
    ...
    /// color loops.
    pub fn substep_integrate_velocities(&self, pass: &mut GpuPass, mb: &mut GpuMultibodySet, args: &mut MultibodySolverArgs<'_>) -> Result<(), GpuBackendError> {
        if mb.is_empty() {
            return Ok(());
        }
        let dispatch = [mb.multibodies_per_batch, mb.num_batches, 1];
    ...
    /// pipeline can interleave multibody phases with the fused free-body
    /// color loops.
    pub fn substep_solve_with_bias(&self, pass: &mut GpuPass, mb: &mut GpuMultibodySet, args: &mut MultibodySolverArgs<'_>) -> Result<(), GpuBackendError> {
        if mb.is_empty() {
            return Ok(());
        }
        let dispatch = [mb.multibodies_per_batch, mb.num_batches, 1];
    ...
    /// pipeline can interleave multibody phases with the fused free-body
    /// color loops.
    pub fn substep_integrate_positions(&self, pass: &mut GpuPass, mb: &mut GpuMultibodySet, args: &mut MultibodySolverArgs<'_>, is_last_substep: bool) -> Result<(), GpuBackendError> {
        if mb.is_empty() {
            return Ok(());
        }
        let dispatch = [mb.multibodies_per_batch, mb.num_batches, 1];
    ...
    /// pipeline can interleave multibody phases with the fused free-body
    /// color loops.
    pub fn substep_solve_no_bias(&self, pass: &mut GpuPass, mb: &mut GpuMultibodySet, args: &mut MultibodySolverArgs<'_>) -> Result<(), GpuBackendError> {
        if mb.is_empty() {
            return Ok(());
        }
        let dispatch = [mb.multibodies_per_batch, mb.num_batches, 1];
