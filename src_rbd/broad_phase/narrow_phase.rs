...
impl GpuNarrowPhase {
    ...
    pub fn dispatch(
        ...
        // Flat 1-D grids: the pair/PFM kernels walk `0..total` and map each item
        // back to its batch via the prefix offsets, so the capacity-based fixed
        // fallback is total capacity over 64, not per-batch-rounded.
