...
//!
//! Run: cargo run -p pendulum_headless --bin pendulum_learn --release [seed]
    ...
    for _ in 0..T {
        {
            ...
            for e in 0..NB {
                let obs = [u[e].x, u[e].z, (u[e].x - prev[e].x) / DT, (u[e].z - prev[e].z) / DT];
...
fn main() {
    ...
    pollster::block_on(async {
        ...
        println!("CEM training: {POP} candidates × {M} envs (batch {NB}), {T}-step rollouts, {GENS} gens");
        println!("policy: linear, {NPARAM} weights (obs=[ux,uz,u̇x,u̇z] → act=[ωx,ωz])\n");
        println!("{:>3}  {:>10}  {:>10}  {:>12}", "gen", "best fit", "elite fit", "elite upright");
        ...
        for g in 0..GENS {
            ...
            let (fit, upf) = evaluate(&gpu, &pipeline, &pop, seed.wrapping_add(g as u64 * 1009)).await;
            ...
            println!(
                "{:>3}  {:>10.3}  {:>10.3}  {:>11.0}%",
                g, fit[idx[0]], elite_fit, elite_up * 100.0
            );
        }

        println!("\nlearned policy (best candidate), action = W·[ux,uz,u̇x,u̇z]:");
        println!("  ωx = {:+.2}·ux {:+.2}·uz {:+.2}·u̇x {:+.2}·u̇z", best_mean[0], best_mean[1], best_mean[2], best_mean[3]);
        println!("  ωz = {:+.2}·ux {:+.2}·uz {:+.2}·u̇x {:+.2}·u̇z", best_mean[4], best_mean[5], best_mean[6], best_mean[7]);
