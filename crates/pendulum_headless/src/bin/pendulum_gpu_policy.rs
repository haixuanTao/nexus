...
//!
//! Run: cargo run -p pendulum_headless --bin pendulum_gpu_policy --release
...
impl Lcg {
    ...
    fn range(&mut self, a: f32, b: f32) -> f32 { a + (b - a) * self.unit() }
}
...
async fn webgpu_backend() -> KhalGpuBackend {
    let limits = wgpu::Limits {
        max_buffer_size: 1_200_000_000, max_storage_buffer_binding_size: 1_200_000_000,
        max_storage_buffers_per_shader_stage: 14, max_compute_workgroup_storage_size: 19_904,
        ..Default::default()
    };
    let mut w = WebGpu::new(wgpu::Features::default(), limits).await.expect("webgpu");
...
fn build_env(rng: &mut Lcg) -> (RigidBodySet, ColliderSet, MultibodyJointSet) {
    ...
    let rod = bodies.insert(RigidBodyBuilder::dynamic().translation(u * 2.0).rotation(rot.to_scaled_axis()));
    colliders.insert_with_parent(ColliderBuilder::cuboid(1.0, 0.12, 0.12).density(1.0), rod, &mut bodies);
    let mut joint = SphericalJointBuilder::new().local_anchor1(Vec3::ZERO).local_anchor2(Vec3::new(-2.0, 0.0, 0.0));
    for axis in [JointAxis::AngX, JointAxis::AngZ] {
        joint = joint.motor_velocity(axis, 0.0, 50.0).motor_max_force(axis, 60.0);
    }
...
fn main() -> anyhow::Result<()> {
    pollster::block_on(async {
        ...
        for c in 0..NDATA {
            let o = [rng.range(-0.6, 0.6), rng.range(-0.6, 0.6), rng.range(-2.0, 2.0), rng.range(-2.0, 2.0)];
            let a = expert(o);
            for r in 0..IN { xd[(r, c)] = o[r]; }
            for r in 0..OUT { td[(r, c)] = a[r]; }
        }
        let gx = mk(&xd, st);
        let gt = mk(&td, st);

        // params (no bias), small init
        let mut w1 = mk(&DMatrix::from_fn(HID, IN, |r, c| (((r * IN + c) % 7) as f32 * 0.05 - 0.15)), st);
        let mut w2 = mk(&DMatrix::from_fn(OUT, HID, |r, c| (((r * HID + c) % 5) as f32 * 0.05 - 0.1)), st);
        // activations / grads
        ...
        let (mut mw1, mut vw1) = (mk(&DMatrix::zeros(HID, IN), st), mk(&DMatrix::zeros(HID, IN), st));
        let (mut mw2, mut vw2) = (mk(&DMatrix::zeros(OUT, HID), st), mk(&DMatrix::zeros(OUT, HID), st));

        println!("GPU policy training (vortx): MLP {IN}-{HID}(tanh)-{OUT}, {NDATA} samples, {TRAIN_STEPS} steps");
        println!("cloning the 2-DOF balance controller; forward+backward+Adam all on GPU\n");
        let (b1, b2, eps) = (0.9f32, 0.999f32, 1e-8f32);
        let mut first = 0.0;
        for t in 1..=TRAIN_STEPS {
            let mut enc = backend.begin_encoding();
            { let mut p = enc.begin_pass("z1", None); gemm.dispatch_naive(&backend, &mut sh, &mut p, &mut z1, &w1, &gx)?; }
            { let mut p = enc.begin_pass("tanh", None); act.tanh(&backend, &mut sh, &mut p, &mut z1)?; }
            { let mut p = enc.begin_pass("z2", None); gemm.dispatch_naive(&backend, &mut sh, &mut p, &mut z2, &w2, &z1)?; }
            ...
            let gp = Tensor::scalar(&backend, AdamParams { lr: LR, beta1: b1, beta2: b2, eps, bias_correction1: bc1, bias_correction2: bc2, pad0: 0.0, pad1: 0.0 }, BufferUsages::UNIFORM)?;
            let mut enc = backend.begin_encoding();
            { let mut p = enc.begin_pass("dW2", None); gemm.dispatch_naive(&backend, &mut sh, &mut p, &mut dw2, &z2, z1.transpose_last_dims())?; }
            { let mut p = enc.begin_pass("da1", None); gemm.dispatch_naive(&backend, &mut sh, &mut p, &mut da1, w2.transpose_last_dims(), &z2)?; }
            { let mut p = enc.begin_pass("dz1", None); act.tanh_backward(&backend, &mut sh, &mut p, &mut da1, &z1)?; }
            { let mut p = enc.begin_pass("dW1", None); gemm.dispatch_naive(&backend, &mut sh, &mut p, &mut dw1, &da1, gx.transpose_last_dims())?; }
            { let mut p = enc.begin_pass("adam_w2", None); adam.step(&backend, &mut sh, &mut p, &gp, &mut w2, &dw2, &mut mw2, &mut vw2)?; }
            { let mut p = enc.begin_pass("adam_w1", None); adam.step(&backend, &mut sh, &mut p, &gp, &mut w1, &dw1, &mut mw1, &mut vw1)?; }
            backend.submit(enc)?;
            backend.synchronize()?;
        }
        ...
        for _ in 0..nenv { let (b, c, m) = build_env(&mut es); bs.push(b); cs.push(c); ms.push(m); }
        let imp = ImpulseJointSet::new();
        let mut sp = GpuSimParams::default(); sp.dt = 1.0 / 60.0; sp.num_solver_iterations = 8;
        let envs: Vec<_> = (0..nenv).map(|i| (&bs[i], &cs[i], &imp, &ms[i], &sp)).collect();
        let mut state = GpuPhysicsState::from_rapier(&backend, &envs);
        let stride = state.num_colliders_per_batch() as usize;
        let pole = |p: &[Pose], e: usize| { let t = p[e * stride + 1].translation; Vec3::new(t.x, t.y, t.z).normalize() };
        ...
        let fwd = |o: [f32; IN], col_major: bool| {
            ...
            for h in 0..HID {
                let mut s = 0.0;
                for i in 0..IN { s += if col_major { w1v[i * HID + h] } else { w1v[h * IN + i] } * o[i]; }
                a1[h] = s.tanh();
            }
            let mut out = [0.0f32; OUT];
            for k in 0..OUT {
                let mut s = 0.0;
                for h in 0..HID { s += if col_major { w2v[h * OUT + k] } else { w2v[k * HID + h] } * a1[h]; }
        ...
        for _ in 0..32 {
            let o = [probe.range(-0.5, 0.5), probe.range(-0.5, 0.5), probe.range(-1.5, 1.5), probe.range(-1.5, 1.5)];
            let e = expert(o);
            let (cm, rm) = (fwd(o, true), fwd(o, false));
            for k in 0..OUT { err_cm += (cm[k] - e[k]).powi(2); err_rm += (rm[k] - e[k]).powi(2); }
        }
        let col_major = err_cm <= err_rm;
        println!("weight layout: {} (err col={err_cm:.3} row={err_rm:.3})", if col_major { "column-major" } else { "row-major" });
        ...
        for _ in 0..steps {
            { let mm = state.multibodies_mut();
              for e in 0..nenv {
                  let o = [u[e].x, u[e].z, (u[e].x - prev[e].x) * 60.0, (u[e].z - prev[e].z) * 60.0];
                  let a = mlp(o);
            ...
            for e in 0..nenv { u[e] = pole(&p, e); if u[e].y.clamp(-1.0, 1.0).acos() < tol { up += 1; } }
        }
        println!("deployed GPU-trained policy on {nenv} nexus envs: upright {:.0}% of {steps} steps",
