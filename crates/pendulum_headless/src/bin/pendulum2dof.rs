...
//!
//! Run: cargo run -p pendulum_headless --bin pendulum2dof --release [out.csv]
...
async fn run(label: &str, controlled: bool, out_csv: Option<&str>) -> f32 {
    ...
    for step in 1..=STEPS {
        ...
        if step % 60 == 0 || step == STEPS {
            ...
            println!("  [{label}] step {step:>3}: tilt-from-up = {tilt:>5.1}°  u=({:.2},{:.2},{:.2})", u.x, u.y, u.z);
        }
    }
    ...
    println!("[{label}] upright {balanced}/{STEPS} = {:.0}% ({:.2}s)", frac * 100.0, frac * STEPS as f32 * DT);
    frac
}
...
fn main() {
    ...
    pollster::block_on(async {
        ...
        println!("\n2-DOF inverted pendulum — controlled {:.0}% vs baseline {:.0}%", c * 100.0, b * 100.0);
    });
}
