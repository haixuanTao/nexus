...
//!
//! Run: cargo run -p pendulum_headless --bin pendulum_ppo --release [seed]
...
struct Mlp { ni: usize, nh: usize, no: usize, w1: Vec<f32>, b1: Vec<f32>, w2: Vec<f32>, b2: Vec<f32> }
struct MlpGrad { w1: Vec<f32>, b1: Vec<f32>, w2: Vec<f32>, b2: Vec<f32> }
impl MlpGrad {
    fn zero(m: &Mlp) -> Self {
        MlpGrad { w1: vec![0.0; m.nh * m.ni], b1: vec![0.0; m.nh], w2: vec![0.0; m.no * m.nh], b2: vec![0.0; m.no] }
    }
}
impl Mlp {
    ...
    fn forward(&self, x: &[f32]) -> (Vec<f32>, Vec<f32>) {
        ...
        for j in 0..self.nh {
            let mut z = self.b1[j];
            for i in 0..self.ni { z += self.w1[j * self.ni + i] * x[i]; }
            h[j] = z.tanh();
        }
        let mut o = vec![0.0f32; self.no];
        for k in 0..self.no {
            let mut z = self.b2[k];
            for j in 0..self.nh { z += self.w2[k * self.nh + j] * h[j]; }
            o[k] = z;
        }
        (o, h)
    }
    fn backward(&self, x: &[f32], h: &[f32], g_out: &[f32], g: &mut MlpGrad) {
        ...
        for j in 0..self.nh {
            ...
            for i in 0..self.ni { g.w1[j * self.ni + i] += dz * x[i]; }
            g.b1[j] += dz;
        }
    }
}

struct Adam { m: Vec<Vec<f32>>, v: Vec<Vec<f32>>, t: i32 }
impl Adam {
    fn new(net: &Mlp) -> Self {
        let shapes = [net.w1.len(), net.b1.len(), net.w2.len(), net.b2.len()];
        Adam { m: shapes.iter().map(|&n| vec![0.0; n]).collect(), v: shapes.iter().map(|&n| vec![0.0; n]).collect(), t: 0 }
    }
...
fn build_env(rng: &mut Lcg) -> (RigidBodySet, ColliderSet, MultibodyJointSet) {
    ...
    let rod = bodies.insert(RigidBodyBuilder::dynamic().translation(u * ROD_LEN).rotation(rot.to_scaled_axis()));
    colliders.insert_with_parent(ColliderBuilder::cuboid(ROD_LEN * 0.5, 0.12, 0.12).density(1.0), rod, &mut bodies);
    let mut joint = SphericalJointBuilder::new().local_anchor1(Vec3::ZERO).local_anchor2(Vec3::new(-ROD_LEN, 0.0, 0.0));
    for axis in [JointAxis::AngX, JointAxis::AngZ] {
        joint = joint.motor_velocity(axis, 0.0, MOTOR_DAMPING).motor_max_force(axis, MOTOR_MAX_FORCE);
    }
...
async fn read_poses(gpu: &KhalGpuBackend, state: &GpuPhysicsState) -> Vec<Pose> {
    gpu.slow_read_vec(state.poses().buffer()).await.expect("poses")
}
...
fn main() {
    ...
    pollster::block_on(async {
        ...
        println!("PPO swing-up: policy {NI}-{NH}-{NA}, value {NI}-{NH}-1, σ={SIGMA}, clip={CLIP}, {EPOCHS} epochs/iter");
        println!("batch {NB} × {T} steps, {ITERS} iters; start hanging (tilt 130–180°), reward=cos(tilt)\n");
        println!("{:>4}  {:>9}  {:>9}  {:>9}", "iter", "mean rew", "upright", "final up");
        ...
        for it in 0..ITERS {
            ...
            for _ in 0..NB { let (b, c, m) = build_env(&mut es); bs.push(b); cs.push(c); ms.push(m); }
            let impulse = ImpulseJointSet::new();
            let mut sp = GpuSimParams::default(); sp.dt = DT; sp.num_solver_iterations = 8;
            let envs: Vec<_> = (0..NB).map(|i| (&bs[i], &cs[i], &impulse, &ms[i], &sp)).collect();
            ...
            for t in 0..T {
                {
                    ...
                    for e in 0..NB {
                        ...
                        let mut a = [0.0f32; NA];
                        for k in 0..NA { a[k] = (mu[k] + SIGMA * rng.gauss()).clamp(-VMAX, VMAX); }
                        let i = e * T + t;
                        obs[i] = x; act[i] = a; logp_old[i] = logp_gauss(&a, &mu); val[i] = v[0];
            ...
            for a in &adv { m += *a; } m /= ns as f32;
            for a in &adv { s += (*a - m) * (*a - m); }
            let sd = (s / ns as f32).sqrt().max(1e-4);
            for a in adv.iter_mut() { *a = (*a - m) / sd; }
