...
//!
//! Run: cargo run -p pendulum_headless --bin pendulum_pg --release [seed]
...
struct Mlp {
    w1: Vec<f32>, b1: Vec<f32>, // NH×NI, NH
    w2: Vec<f32>, b2: Vec<f32>, // NO×NH, NO
}
struct Grad {
...
impl Grad {
    fn zero() -> Self {
        Grad { w1: vec![0.0; NH * NI], b1: vec![0.0; NH], w2: vec![0.0; NO * NH], b2: vec![0.0; NO] }
    }
}
...
struct Adam {
    mw1: Vec<f32>, vw1: Vec<f32>, mb1: Vec<f32>, vb1: Vec<f32>,
    mw2: Vec<f32>, vw2: Vec<f32>, mb2: Vec<f32>, vb2: Vec<f32>,
    t: i32,
}
impl Adam {
    fn new() -> Self {
        Adam {
            mw1: vec![0.0; NH * NI], vw1: vec![0.0; NH * NI], mb1: vec![0.0; NH], vb1: vec![0.0; NH],
            mw2: vec![0.0; NO * NH], vw2: vec![0.0; NO * NH], mb2: vec![0.0; NO], vb2: vec![0.0; NO],
            t: 0,
        }
    }
...
fn build_env(rng: &mut Lcg) -> (RigidBodySet, ColliderSet, MultibodyJointSet) {
    ...
    let rod = bodies.insert(
        RigidBodyBuilder::dynamic().translation(u * ROD_LEN).rotation(rot.to_scaled_axis()),
    );
...
fn main() {
    ...
    pollster::block_on(async {
        ...
        println!("REINFORCE: MLP {NI}-{NH}(tanh)-{NO}, σ={SIGMA}, lr={LR}, batch {NB}, {T}-step rollouts, {ITERS} iters\n");
        ...
        for it in 0..ITERS {
            ...
            for t in 0..T {
                {
                    ...
                    for e in 0..NB {
                        let x = [u[e].x, u[e].z, (u[e].x - prev[e].x) / DT, (u[e].z - prev[e].z) / DT];
            ...
            for e in 0..NB {
                for t in 0..T {
                    ...
                    let a = adv[i];
                    let g_mu = [-(a * noise[i][0] * inv) * scale, -(a * noise[i][1] * inv) * scale];
        ...
        println!("\ntrained MLP policy ({} weights) — gradient-based, on the batched GPU env.", NH * NI + NH + NO * NH + NO);
    });
}
