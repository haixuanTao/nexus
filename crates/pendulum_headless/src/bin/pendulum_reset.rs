...
//!
//! Run: cargo run -p pendulum_headless --bin pendulum_reset --release

use khal::backend::{GpuBackend as KhalGpuBackend, WebGpu, Backend};
...
fn build_env(u: Vec3) -> (RigidBodySet, ColliderSet, MultibodyJointSet) {
    ...
    let joint = SphericalJointBuilder::new().local_anchor1(Vec3::ZERO).local_anchor2(Vec3::new(-ROD_LEN, 0.0, 0.0));
...
fn build_state<'a>(
    gpu: &KhalGpuBackend,
    dirs: &[Vec3],
    keep: &'a mut (Vec<RigidBodySet>, Vec<ColliderSet>, Vec<MultibodyJointSet>, ImpulseJointSet, nexus3d::rbd::dynamics::GpuSimParams),
) -> GpuPhysicsState {
    for u in dirs {
        let (b, c, m) = build_env(*u);
        keep.0.push(b); keep.1.push(c); keep.2.push(m);
    }
    keep.4.dt = DT;
    keep.4.num_solver_iterations = 8;
    let envs: Vec<_> = (0..dirs.len()).map(|i| (&keep.0[i], &keep.1[i], &keep.3, &keep.2[i], &keep.4)).collect();
...
fn main() {
    pollster::block_on(async {
        ...
        let pole = |p: &[Pose], e: usize| { let t = p[e * 2 + 1].translation; Vec3::new(t.x, t.y, t.z).normalize() };

        // Reference: 4-env batch, env0 = config S from the start.
        let mut rk = (vec![], vec![], vec![], ImpulseJointSet::new(), Default::default());
        let ref_dirs = [us, dir(0.5, 1.0), dir(0.4, 2.0), dir(0.6, 3.0)];
        let mut rstate = build_state(&gpu, &ref_dirs, &mut rk);
        let mut refs = Vec::new();
        for _ in 0..POST { let _ = pipeline.step(&gpu, &mut rstate, None).await; gpu.synchronize().unwrap(); pipeline.auto_resize_buffers(&gpu, &mut rstate).await; refs.push(pole(&read(&gpu, &rstate).await, 0)); }

        // Test batch: different env0 start; run PRE steps, then reset env0 -> S.
        let mut bk = (vec![], vec![], vec![], ImpulseJointSet::new(), Default::default());
        let b_dirs = [dir(0.9, 4.5), dir(0.5, 1.0), dir(0.4, 2.0), dir(0.6, 3.0)];
        let mut bstate = build_state(&gpu, &b_dirs, &mut bk);
        for _ in 0..PRE { let _ = pipeline.step(&gpu, &mut bstate, None).await; gpu.synchronize().unwrap(); pipeline.auto_resize_buffers(&gpu, &mut bstate).await; }
        let env1_before = pole(&read(&gpu, &bstate).await, 1);

        // build a single-env state for config S, reset env0 from it
        let mut sk = (vec![], vec![], vec![], ImpulseJointSet::new(), Default::default());
        ...
        println!("env0 target dir   = ({:+.3},{:+.3},{:+.3})", us.x, us.y, us.z);
        println!("env0 after reset  = ({:+.3},{:+.3},{:+.3})  (should match target)", env0_reset.x, env0_reset.y, env0_reset.z);
        println!("env1 reset-induced change = {:.5}  (should be ~0)", (env1_after - env1_before).length());

        // step both and compare env0 trajectories
        let mut max_err: f32 = 0.0;
        for k in 0..POST {
            let _ = pipeline.step(&gpu, &mut bstate, None).await; gpu.synchronize().unwrap(); pipeline.auto_resize_buffers(&gpu, &mut bstate).await;
        ...
        println!("{}", if max_err < 1e-2 { "RESET CONSISTENT ✓ (reset env evolves identically to a fresh one)" } else { "INCONSISTENT ✗ (missing state buffer in reset)" });
    });
}
